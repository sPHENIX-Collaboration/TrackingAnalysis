...
For advanced users, you can optimize the ONNX model further:

```bash
# Quantize to INT8 (reduces size, increases speed, slight accuracy loss)
python -m onnxruntime.quantization.quantize_dynamic \
    unet3d_tpc.onnx \
    unet3d_tpc_quantized.onnx \
    --per_channel

# Optimize for inference
python -m onnxruntime.tools.symbolic_shape_infer \
    --input unet3d_tpc.onnx \
    --output unet3d_tpc_optimized.onnx
```
...
|------|------|-------------|
| `unet3d_tpc.onnx` | 21.6 MB | Standard ONNX model |
| `unet3d_tpc_norm_params.txt` | 540 B | Normalization parameters |
| PyTorch checkpoint | 67 MB | Original PyTorch model |

The ONNX model is ~3x smaller due to:
- Removal of training-specific layers
- Weight quantization during export
- Optimization passes
...
**Step 3 (Inference):** Use the ONNX model to perform inference on TPC volumes from Step 1

See `../3_inference/README.md` for C++ inference usage.
...
```
Error: Cannot load unet3d_weighted_normalized_ddp.pt
```

**Solution:** Ensure the PyTorch checkpoint is in the parent directory or specify the path with `--model-path`.

### Export fails with shape mismatch

```
Error: Shape mismatch during export
```

**Solution:** Verify the model architecture matches the checkpoint. The model expects:
- Input: `[1, 1, 205, 66, 160]`
- Output: `[1, 6, 42, 28, 42]`

### Missing normalization parameters

```
KeyError: 'target_mean'
```

**Solution:** Your checkpoint must include `target_mean` and `target_std` tensors. If they're missing, you need to retrain or manually add them to the checkpoint.
...
- **Model Training:** See `../USAGE.md` in parent directory
