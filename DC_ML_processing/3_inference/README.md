...
## Usage with ROOT (Recommended)

### Option 1: Interactive ROOT Session
...
Run non-interactively from command line:

```bash
root -l -b -q 'TPCInference.C+("../2_model_export/unet3d_tpc.onnx", "../dc_r_phi_tpcvolume.root", "distortions_output.root")'
```

The `-b` flag runs batch mode, `-q` quits after execution.

### Option 3: Test with Dummy Data

To test without input data:

```bash
root -l
.L TPCInference.C+
RunInference("../2_model_export/unet3d_tpc.onnx")  // Uses random data
```

## Usage as Standalone C++

Compile the standalone example:

```bash
g++ -o tpc_inference tpc_inference_example.cpp \
...
```bash
# With dummy data
./tpc_inference ../2_model_export/unet3d_tpc.onnx

# With input ROOT file
./tpc_inference ../2_model_export/unet3d_tpc.onnx ../dc_r_phi_tpcvolume.root distortions_output.root
```
...
Adjust the number of threads in the code:

```cpp
// In TPCInference.C or tpc_inference_example.cpp
Ort::SessionOptions session_options;
session_options.SetIntraOpNumThreads(8);  // Adjust based on your CPU
```

Recommended values:
- Desktop: 4-8 threads
- HPC node: 16-32 threads
- Laptop: 2-4 threads
...
```cpp
// Add CUDA execution provider (requires CUDA-enabled ONNX Runtime)
OrtCUDAProviderOptions cuda_options;
cuda_options.device_id = 0;
session_options.AppendExecutionProvider_CUDA(cuda_options);
```

This can provide 10-50x speedup for large batches.

### Batch Processing

To process multiple TPC volumes in parallel:

```cpp
// Modify input shape for batch processing
std::vector<int64_t> input_shape = {BATCH_SIZE, 1, 205, 66, 160};

// Process multiple events in one inference call
```

## Validation

Verify your inference results:
...
r_vals_nz->Draw()  // Should show smooth spatial distribution

// Print statistics
r_vals_nz->GetMean()   // Should be ~1.0-1.5 cm
r_vals_nz->GetRMS()    // Should be ~0.5 cm

h_vals_nz->GetMean()   // Should be ~-0.01 cm
z_vals_nz->GetMean()   // Should be ~-0.04 cm
```

Expected distortion magnitudes:
- **r distortions:** Largest (~0.5-1.5 cm)
- **z distortions:** Moderate (~0.02-0.05 cm)
- **φ (h) distortions:** Smallest (~0.01 cm)

## Performance Benchmarks

| Operation | CPU Time | GPU Time* | Memory |
|-----------|----------|-----------|--------|
| Load model | ~1 sec | ~2 sec | ~500 MB |
| Single inference | ~2-5 sec | ~0.1-0.5 sec | ~1 GB |
| Save output | ~0.5 sec | ~0.5 sec | ~50 MB |

*GPU times require CUDA-enabled ONNX Runtime
...
Or use verbose compilation:
```cpp
.L TPCInference.C++g  // Force rebuild with debug info
```

### Wrong histogram name

```
Error: Cannot find histogram 'dc_values'
```

**Solution:** Verify your input ROOT file contains a histogram named `dc_values`. Check with:
```bash
root -l input.root
.ls
```

### Normalization parameter file not found

```
Error: Cannot open unet3d_tpc_norm_params.txt
```

**Solution:** Ensure the normalization parameter file exists at:
`../2_model_export/unet3d_tpc_norm_params.txt`

Or modify the path in the code:
```cpp
// In TPCInference.C
std::ifstream param_file("/full/path/to/unet3d_tpc_norm_params.txt");
```

### Unexpected output values

If distortion values seem wrong:
1. Check that input normalization is applied (z-score per sample)
2. Verify normalization parameters are correct
3. Ensure input histogram has correct dimensions (205×66×160)
4. Verify model file is from Step 2 export
...
# 2. Navigate to inference directory
cd /path/to/sphenix_tpc/onnx/3_inference

# 3. Run inference in ROOT
root -l -b -q 'TPCInference.C+("../2_model_export/unet3d_tpc.onnx", "../dc_r_phi_tpcvolume.root", "distortions_output.root")'
...
```cpp
// In ROOT session
r_vals_nz->Draw("colz")  // Visualize radial distortions
```

## Integration with sPHENIX Reconstruction

To use these distortion maps in track reconstruction:

1. Load the distortion histograms in your reconstruction macro
2. For each track hit at position (r, φ, z):
   - Look up distortion values from the 6 histograms
   - Apply corrections: `corrected_pos = measured_pos - distortion`
3. Use corrected positions for track fitting

Example integration:
```cpp
// In your reconstruction code
TFile* dist_file = TFile::Open("distortions_output.root");
TH3F* r_dist_nz = (TH3F*)dist_file->Get("r_vals_nz");
// ... load other components ...

// During hit processing
float dr = r_dist_nz->Interpolate(z, r, phi);  // Get distortion
float corrected_r = measured_r - dr;           // Apply correction
```

## Next Steps

- **Validation:** Compare predictions with ground truth using Python scripts in parent directory
- **Optimization:** Implement batch processing for multiple events

## References

- **ONNX Runtime C++ API:** https://onnxruntime.ai/docs/api/c/
- **ROOT Histogram Documentation:** https://root.cern/doc/master/classTH3.html
- **Step 1 (Preprocessing):** See `../1_dc_preprocessing/README.md`
- **Step 2 (Model Export):** See `../2_model_export/README.md`
