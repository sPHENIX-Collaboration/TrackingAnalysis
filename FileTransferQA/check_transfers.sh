...
root -b -q CheckFileTransfer.C\(\"${yesterday}\"\) >& ${logfile}
fullruns=$(grep  'Complete runs (all segments):' ${logfile})
allseginbraces=$(echo "${fullruns}" | sed -n 's/.*{ *\([^}]*\)*}.*/\1/p')
root -b -q CheckFileTransfer.C\(\"${yesterday}\",true\) >& ${logfile}

fullruns=$(grep "Complete runs (first segment):" ${logfile})
zeroseginbraces=$(echo "${fullruns}" | sed -n 's/.*{ *\([^}]*\) *}.*/\1/p')
