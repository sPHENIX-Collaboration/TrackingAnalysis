...
void plot_kshorts_kfp()
{
  ...
  ntp_unlikesign->Draw("track_1_track_2_DCA>>hpdca", sicut+ptcut+"K_S0_DIRA>0.9 && pow(K_S0_decayLength,2)>0.2");
  hpdca->Draw();

  c1->cd(3);
  TH1D *hcosth = new TH1D("hcosth","cosThetaReco",100,0.5,1);
  ntp_unlikesign->Draw("K_S0_DIRA>>hcosth", sicut+ptcut+"pow(K_S0_decayLength,2)>0.2");
  hcosth->Draw();
