...
#include <ModuleEdge/PHModuleEdgeCorresctions.h>
#include <stdio.h>
...
R__LOAD_LIBRARY(install/lib/libModuleEdge.so)
R__LOAD_LIBRARY(libtrackingqa.so)



void Fun4All_FieldOnAllTrackers(
    const std::string tpcfilename = "DST_STREAMING_EVENT_run2pp_new_2024p002-00051109-00000.root",
    ...
    const int nEvents = 0)
{
  ...
  TString outfile = outfilename + "_" + runnumber + "_" + segment;
...
//  Micromegas_HitUnpacking();
  ...
  se->registerSubsystem(finder);
      ...
      auto residuals = new PHModuleEdgeCorresctions;
      const TString tpc_residoutfile = theOutfile + "_PhTpcResiduals.root";
      const TString tpc_correctio_noutfile = theOutfile + "_ModuleEdgeCorrectionMap1.root";
      residuals->setOutputfile(tpc_residoutfile.Data());
      residuals->setMinPt( 0.2 );
      residuals->setMaxQuality( 100. );
      residuals->setMaxRPhiResidual( 1. );
      residuals->setRequiredTpcClusters( 30 );
      residuals->setDoResidualFit( true );
      residuals->CreateCorrectionMap(false);
      residuals->setCorrectionFile(tpc_correction_outfile.Data());
      residuals->setExistingCorrectionFile("");
