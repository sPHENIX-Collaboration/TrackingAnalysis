...
```
Currently, it supports KFParticle QA for reconstructing four resonances: $K_S^0,\ \Lambda,\ \phi,\ D^0$, conversion photon as well as various Tracking QA modules which depends on which level of DST you input.
...
```
If you want get trigger info, detector info, dEdx info, calo info in your output nTuples please enable these lines:
```
...
If want to get momentum in the secondary vertex for asymmetry study, set this flag to false
```c++
  bool extraolate_tracks_to_secondary_vertex = false;
```
Otherwise, for general analysis purpose, set it to true so that the track momentum is extraolated to the secondary vertex
