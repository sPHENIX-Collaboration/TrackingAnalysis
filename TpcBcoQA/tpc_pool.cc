...
{
  if(min_depth() < _required_depth){
    std::cout << "min_depth() : " << min_depth() << ".. whereas _required_depth : " << _required_depth << std::endl;
  }
...
int tpc_pool::tpc_decode()
{
  ...
  std::set<unsigned int> unique_fee_ids;
  NR_VALIDFEE=0;
  // demultiplexer
  while (index < payload_length)
  {
    ...
    if ((_thebuffer[index] & 0xFF00) == FEE_MAGIC_KEY)
    {
      ...
      if (unique_fee_ids.insert(fee_id).second) NR_VALIDFEE++;
